...
const CandlestickChart = ({
  ...
  liveInterval,
  setLiveInterval,
}: any) => { // Replace 'any' with your CandlestickChartProps interface
  ...
  const fetchOHLCData = async (selectedPeriod: string) => {
    try {
      const config = PERIOD_CONFIG[selectedPeriod];
      
      // ✅ Build clean params for Demo API
      ...
      // Only send interval if it's explicitly defined (and valid)
      // For Demo API, it's safer to let CoinGecko decide based on 'days'
      const newData = await fetcher<any[]>(`/coins/${coinId}/ohlc`, queryParams);
  ...
  useEffect(() => {
    ...
    const convertedToSeconds = ohlcData.map(
      (item) => [Math.floor(item[0] / 1000), item[1], item[2], item[3], item[4]] as any,
    );
  ...
  }, [height, period, ohlcData]); // Added ohlcData to dependency to refresh on fetch
  ...
  useEffect(() => {
    ...
    if (prevOhlcDataLength.current !== ohlcData.length || mode === 'historical') {
